...
class Monitor:
    ...
    def _average_total_distance(self) -> float:
        ...
        for act in self._activities[DRIVER].values():
            i = 0
            while i < len(act) - 1:
                total += abs(act[i].location.row - act[i + 1].location.row)
                total += abs(act[i].location.column
                             - act[i + 1].location.column)
                i += 1
        return total / len(self._activities[DRIVER])
    ...
    def _average_ride_distance(self) -> float:
        ...
        for act in self._activities[DRIVER].values():
            for ac in act:
                if ac.description == PICKUP or ac.description == DROPOFF:
                    stored.append(ac.location)
        i = 0
        while i < len(stored) - 1:
            total += abs(stored[i].row - stored[i + 1].row)
            total += abs(stored[i].column - stored[i + 1].column)
            i += 2
        return total / len(self._activities[DRIVER])
